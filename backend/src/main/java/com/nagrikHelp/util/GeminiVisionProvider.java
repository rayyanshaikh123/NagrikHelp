...
public class GeminiVisionProvider implements VisionModelProvider {
    ...
    public VisionResult analyze(String imageBase64) throws Exception {
        ...
        // Try official client first only if env/API key present
        if (apiKey != null && !apiKey.isBlank()) {
            try {
                Client client = new Client(); // reads GEMINI_API_KEY env internally
                GenerateContentResponse resp = client.models.generateContent(modelId, fullPrompt, null);
                String text = resp.text();
                VisionResult parsed = parseGeminiText(text);
                if (parsed != null) return parsed;
                log.debug("[AI] Official client returned unparsable text, falling back to HTTP parser");
            } catch (Throwable t) {
                log.warn("[AI] Official client path failed: {} - falling back to raw HTTP", t.toString());
            }
        }

        // Fallback to raw HTTP (legacy path)
